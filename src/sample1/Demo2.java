...
public class Demo2 extends Application {
	 ...
	 * 장면(Scene)를 생성해서 stage에 설정하기
	 */
	@Override
	public void start(Stage primaryStage) {
		VBox rootLayout = new VBox(); //루트 컨테이너 생성
		...
		btn.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				System.out.println("확인버튼 클릭 클릭 클릭");
				
			}
		});//이벤트 핸들러는 인터페이스 인터페이스가 와야함.
		*/
		btn.setOnAction((event)->{
			...
			alert.setContentText("확인 버튼이 클릭되었습니다");
			alert.showAndWait();
		});
